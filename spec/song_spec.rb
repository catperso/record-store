...
describe '#Song' do
  ...
  describe('#==') do
    it("is the same song if it has the same attributes as another song") do
      ...
      song2 = Song.new("Naima", @album.id, nil, nil)
  ...
  describe('.all') do
    it("returns a list of all songs") do
      song = Song.new("Giant Steps", @album.id, nil, nil)
      song.save()
      song2 = Song.new("Naima", @album.id, nil, nil)
      song2.save()
  ...
  describe('#update') do
    it("updates an song by id") do
      song = Song.new("Naima", @album.id, nil, "lyric")
      song.save()
      song.update("Mr. P.C.", @album.id, "lyric")
  ...
  describe('.find_by_album') do
    it("finds songs for an album") do
      ...
      song = Song.new("Naima", @album.id, nil, nil)
      song.save()
      song2 = Song.new("California", album2.id , nil, nil)
      song2.save()
  ...
  describe('#song_sort') do
    it('sorts songs alphabetically') do
      song = Song.new("Banana", nil, nil, nil)
      song.save()
      song2 = Song.new("Apricot", nil, nil, nil)
      song2.save()
      Song.sort
